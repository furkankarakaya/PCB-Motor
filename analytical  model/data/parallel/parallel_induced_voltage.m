...
an_mag_par=[an_mag_par, zeros(1, length(test_par_fft) - length(an_mag_par))];
an_fr_par=[an_fr_par, zeros(1, length(test_par_freq) - length(an_fr_par))];
%%
an_mag_par=[an_mag_par, zeros(1, length(test_par_fft) - length(an_mag_par))];
...
y=[transpose(an_mag_par(1:50)) magInducedVoltagePhaseA test_par_fft(1:50) ];
bar(x,y);
